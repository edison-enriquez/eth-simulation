...
class Ethereum(mosaik_api.Simulator):
    ...
    def create(self, num, model):
        ...
        for i in range(num):
            ...
            trans_hash = self.contract.functions.addParticipant().transact(temp)
    ...
    def step(self, time, inputs=None):
        ...
        for eid, attrs in inputs.items():
            ...
            trans_hash = self.contract.functions.postEnergyBalance(int(netload)).transact(temp)
            self._wait_until_trans_receipt(trans_hash)
        ...
        trans_hash = self.contract.functions.clearMarket().transact(temp)
        self._wait_until_trans_receipt(trans_hash)
    ...
    def get_data(self, outputs=None):
        ...
        energy_posted_event = contract.events.EnergyPostedEvent.create_filter(fromBlock='earliest')
        market_cleared_event = contract.events.MarketClearedEvent.create_filter(fromBlock='earliest')
        bill_sent_event = contract.events.BillSentEvent.create_filter(fromBlock='earliest')
        participant_at_clearing_request = contract.events.ParticipantAtClearingRequest.create_filter(fromBlock='earliest')
    ...
    def _wait_until_trans_receipt(self, trans_hash):
        ...
        while status is None:
            status = self.web3.eth.get_transaction_receipt(trans_hash)
            sleep(0.1)
